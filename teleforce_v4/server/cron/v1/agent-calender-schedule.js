...
cron.schedule("*/60 * * * * *", async () => {
    ...
    if (hours > 7 && hours < 21) {
        //Email to agent
        db.query('SELECT s.*,a.mobile as agent_mobile,a.created_by as managerid,a.account_name,a.email as account_email,a.emailserver_alloted,es.smtp_server,es.smtp_port,es.smtp_username,es.smtp_password FROM tf_scheduler s  LEFT JOIN tf_account_table a ON s.user_id=a.account_id LEFT JOIN tf_manager_email_server es ON a.emailserver_alloted=es.server_id WHERE s.agent_email_status = 0 AND DATE(s.StartTime)=CURRENT_DATE limit 0,10', [], async function (err, adata) {
            if (err) {
                console.log(err);
            } else {
                ...
                if (adata.length > 0) {
                    ...
                    await async.forEachOf(adata, (data, callback) => {
                        ...
                        var agent_mobile = data['agent_mobile'];
                        ...
                        if (emailserver_alloted > 0 && managerid>0) {
                            db.query('SELECT c.metavalue as scheduledemailtemplate,e.email_html FROM tf_configuration_calender c LEFT JOIN tf_emailtemplate e ON c.metavalue=e.email_id  WHERE c.metakey="scheduledemailtemplate" AND c.user_id = ' + managerid, [], async function (err, tdata) {
                                if (err) {
                                    ...
                                    if (tdata.length > 0) {
                                        ...
                                        if(email_html!=null){
                                            var status = validator.validate(account_email);
                                            console.log("status :" + account_email);
                                            if (status == true) {
                                                var transOptions = {
                                                    host: smtp_server,
                                                    port: smtp_port,
                                                ...
                                                email_html = email_html.replace("{email_id}", email);
                                                email_html = email_html.replace("{agent_name}", account_name);
                                                ...
                                                let info = {
                                                    ...
                                                    subject: "Invitation: " + account_name + " and " + customer_name + " @ " + dateFormat(StartTime, "fullDate") + ',' + dateFormat(StartTime, "h:MM TT") + ' To ' + dateFormat(EndTime, "h:MM TT") + "  (" + email + ")",
                                                ...
                                                transporter.sendMail(info, function (error, info) {
                                                    if (error) {
                                                        ...
                                                        if (info['accepted'].length > 0) {
                                                            db.query('UPDATE tf_scheduler SET `agent_email_status`=1 WHERE s_id=' + s_id);
                                                        }
                                                    }
                                                });
                            ...
                            console.log({ 'Error': 'No Email Template or Manager found.' });
                        }
                    })
                }
            }
        ...
        //sms to agent
        db.query('SELECT s.*,a.created_by as managerid,a.mobile as account_mobile,a.account_name,a.email as account_email,aa.AccessKey as authkey FROM tf_scheduler s  LEFT JOIN tf_account_table a ON s.user_id=a.account_id  LEFT JOIN tf_account_table aa ON aa.account_id = a.created_by WHERE s.agent_sms_status = 0 limit 0,10', [], async function (err, adata) {
            if (err) {
                console.log(err);
            } else {
                console.log("adata.length :" + adata.length);
                                    ...
                                    if (tdata.length > 0) {
                                        ...
                                        request(options, function (error, response) {
                                            if (error) {
                                                ...
                                                db.query('UPDATE tf_scheduler SET `agent_sms_status`=1 WHERE s_id=' + s_id);
                                            }
