...
cron.schedule('*/1 * * * *', () => {
    console.log("Start")
    Erp.AllFiledata('0',function (err, data) {
        if (err) {
                                    ...
                                    if (err) {
                                        ...
                                        if(jsonObj.length>0){
                                            ...
                                            async.forEachOf(jsonObj, async (obj, key, callback) => {
                                                ...
                                                lead.address = obj['address']?obj['address']:'';
                                                lead.city = obj['city']?obj['city']:'';
                                                lead.state = obj['state']?obj['state']:'';
                                                var mobile = obj['mobile']?obj['mobile'].replace(/\D/g,'').substr(-10):'';
                                            ...
                                            Erp.updatefileStatus(data_b, fileid, function (err, count) {
                                                if (err) {
                                                    console.log("Error: " + err)
                                                }
                                                else {
