...
@Component({
  selector: 'app-live-channel3',
  templateUrl: './live-channel3.component.html',
  styleUrls: ['./live-channel3.component.css']
})
export class LiveChannel3Component implements OnInit {
  ...
  getLiveChannel(){
    this.getChannelStat()
    this.isLoading = true;
    this.adminservice.getlivechannel3().then(
      data => {
  ...
  getChannelStat(){
    this.adminservice.getchannelstat3().then(
      data => {
        //console.log(data['data']);
        let statusArr = [];
        let statusvalue = [];
