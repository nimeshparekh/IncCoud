...
@Component({
  ...
  styleUrls: ['./assign-agents.component.css']
})
export class AssignAgentsComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<AssignAgentsComponent>,
  ...
  ngOnInit(): void {
    ...
    this.addForm = this.fb.group({
      ...
      seq_no: [null, [Validators.required]],
      userid: [userid]
    });
  ...
  async AssignAgentsList(userid,groupid) {
    await this.userservice.NotAssignAgentsList(userid,groupid).then(
      data => {
  ...
  submit() {
    ...
    if (this.addForm.invalid == true) {
      ...
      if (this.data != '' && this.data != null && this.data.action == 'add') {
        ...
        this.userservice.saveAssignAgent(data).then(
          data => {
            //console.log(data['data']);
  ...
  async deleteData(id) {
    // console.log(id);   
    await this.userservice.deleteAssignAgent(id).then(
      data => {
        this._snackBar.open(data['data'], '', {
          duration: 2000, verticalPosition: 'top'
        });
