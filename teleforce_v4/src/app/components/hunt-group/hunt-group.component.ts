...
import { CreateHuntGroupComponent } from '../create-hunt-group/create-hunt-group.component';
...
@Component({
  ...
  styleUrls: ['./hunt-group.component.css']
})
export class HuntGroupComponent implements OnInit {

  public dataSource: MatTableDataSource<any>;
  displayedColumns: string[] = ['id','groupid', 'name', 'description', 'transfer_extesion', 'action'];
  Length = 0;
  isLoading =true;
  kyc;
  ...
  ngOnInit(): void {
    ...
    this.getHuntgroup(userid)
  }
  async getHuntgroup(userid) {
  ...
  Createdialog(): void {
    const dialogRef = this.dialog.open(CreateHuntGroupComponent, {
      disableClose: true,
    });
  ...
  copydialog(id): void {
    const dialogRef = this.dialog.open(CopyAgentGroupComponent, {
...
@Component({
  selector: 'copy-agent-group',
  templateUrl: './copy-agent-group.component.html',
  styleUrls: ['./hunt-group.component.css']

})
export class CopyAgentGroupComponent {
