...
@Component({
  ...
  styleUrls: ['./ivr-builder.component.css']
})
export class IvrBuilderComponent implements OnInit {

  public dataSource: MatTableDataSource<any>;
  displayedColumns: string[] = ['id','ivrid', 'name', 'desc','action'];
  Length = 0;
  isLoading = true;
  kyc;
  ...
  Createdialog(): void {
    const dialogRef = this.dialog.open(CreateIvrComponent, {
      width: '100%', disableClose: true,maxHeight:'750px',
    });
  ...
  updatedialog(id): void {
    const dialogRef = this.dialog.open(CreateIvrComponent, {
      width: '100%', disableClose: true,maxHeight:'750px', data: id
    });
    dialogRef.afterClosed().subscribe(result => {
  ...
  copydialog(id): void {
    const dialogRef = this.dialog.open(CopyIvrComponent, {
      width: '100%', disableClose: true,maxHeight:'750px', data: id
    });
    dialogRef.afterClosed().subscribe(result => {
      if(result.event == 'Update'){
...
@Component({
  selector: 'copy-ivr',
  templateUrl: './copy-ivr.component.html',
  styleUrls: ['./ivr-builder.component.css']

})
export class CopyIvrComponent {
  ...
  apitemp = []
  constructor(
    public dialogRef: MatDialogRef<CopyIvrComponent>,
