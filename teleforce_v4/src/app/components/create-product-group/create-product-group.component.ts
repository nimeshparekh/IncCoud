...
export class CreateProductGroupComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<CreateProductGroupComponent>,
  ...
  public ngOnInit(): void {
    ...
    if(this.data!='' && this.data!=null){
      ...
      if(this.data.action=='delete'){
        ...
        this.delagentid = this.data.pg_id
        this.delagentname = this.data.group_name
      }else{
        this.editmode = true
        this.addConForm = this.fb.group({
          
          name: [this.data.group_name, [Validators.required]],
          pg_id: [this.data.pg_id],
        })  
      }
    }else{
      this.addConForm = this.fb.group({
  ...
  submit() {
    ...
    if (this.addConForm.invalid == true) {
      ...
      if (this.data != '' && this.data != null) {
        this.productservice.updatproductgroupData(data).then(
          data => {
        ...
        this.productservice.saveproductgroup(data).then(
          data => {
            this._snackBar.open(data['data'], '', {
              duration: 2000, verticalPosition: 'top'
            });
  ...
  async contactdelete(cont_id){
    await this.productservice.deleteproductgroup(cont_id).then(data => {
      if(data){
