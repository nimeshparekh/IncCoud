...
@Component({
  selector: 'app-livechannels',
  templateUrl: './livechannels.component.html',
  styleUrls: ['./livechannels.component.css']
})
export class LivechannelsComponent implements OnInit {
  ...
  getLiveChannel(){
    this.getChannelStat()
    this.isLoading = true;
    this.adminservice.getlivechannel().then(
      data => {
  ...
  getChannelStat(){
    this.adminservice.getchannelstat().then(
      data => {
        //console.log(data['data']);
        let statusArr = [];
        let statusvalue = [];
