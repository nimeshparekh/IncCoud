...
@Component({
  selector: 'app-manager-reports-dashboard',
  templateUrl: './manager-reports-dashboard.component.html',
  styleUrls: ['./manager-reports-dashboard.component.css']
})
export class ManagerReportsDashboardComponent implements OnInit {
  ...
  public chartDatasetsSource: Array<any> = [
    { data: [0, 10, 20, 90, 20], label: 'Lead Source' }
  ];
  ...
  async hourlycallreportgraph() {
    var userid = localStorage.getItem('access_id');

    await this.reportservice.hourlycallreportgraph(userid).then(
      rdata => {
        if (rdata['data'].length > 0) {
  ...
  async getsalesfunnelreport() {
    ...
    await this.reportservice.getsalesfunnelreport(userid).then(
      rdata => {
    ...
    await this.reportservice.getleadsourcereport(userid).then(
      rdata => {
        ...
        this.chartDatasetsSource = [
          { data: countArr, label: 'Lead Source' }
        ];
