...
@Component({
  selector: 'app-xlnc-agent-performance',
  templateUrl: './xlnc-agent-performance.component.html',
  styleUrls: ['./xlnc-agent-performance.component.css']
})
export class XlncAgentPerformanceComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'agentname', 'logintime', 'talktime', 'wraptime', 'acw','holdtime', 'breaktime', 'totaltime','AHT','inboundcall','outboundcall','outboundconnectcall','missedcall'];
  Length = 0;
  kyc
  data = [];
  loggedtime = [];
  ...
  async searchData() {
    ...
    this.enddate = new Date(data.enddate);
      ...
      if(agentrole=="supervisor"){
        await this.reportservice.supervisior_xlncagentPerformanceData(data).then(
          rdata => {
      ...
      await this.reportservice.XLNCagentPerformanceData(data).then(
        rdata => {
